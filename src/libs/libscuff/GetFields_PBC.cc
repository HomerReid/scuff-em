...
typedef struct GRPIntegrandData
 { 
   ...
   double *kBloch;        // bloch wavevector 
...
{ 
  ...
  if (GBarInterp)
   ...
   { GBarVDEwald(XmX0, K, kBloch, LBV, -1.0, 0, GBarVD);
     Phi        = GBarVD[0];
     GradPhi[0] = GBarVD[1];
     GradPhi[1] = GBarVD[2];
     GradPhi[2] = GBarVD[3];
...
void GetReducedPotentials(RWGObject *O, int ne, const double *X, 
                          cdouble K, double *kBloch, double **LBV, 
                          Interp3D *GBarInterp,
   ...
   { 
     ...
     for(ne=0; ne<O->NumEdges; ne++)
      { 
        ...
        GetReducedPotentials(O, ne, X, K, kBloch, LBV, GBarInterp, a, Curla, Gradp);
...
ObjectIndex=GetFieldObject;
     ...
     /*--------------------------------------------------------------*/
     if (KN)
      GetScatteredFields(PG, X, ObjectIndex, KN, Omega, kBloch, Eps, Mu, GBarInterp, EH);
     else
...
{ 
  ...
  /***************************************************************/
  G->UpdateIncFields(IF, Omega, kBloch);
...
/***************************************************************/
/***************************************************************/
void RWGGeometry::GetFields(IncField *IF, HVector *KN, 
                            cdouble Omega, double *kBloch, 
                            double *X, cdouble *EH)
{
