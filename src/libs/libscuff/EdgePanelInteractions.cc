...
nCalls++;
  ...
  if (WhichCase==1) // common-vertex case 
   ...
   { 
     ...
     zfval[0] = exp(IK*R1)*(2.0*ExpRel(1,-IK*R1) - 0.5*(y+1.0)*ExpRel(2,-IK*R1)) / (8.0*M_PI*R1)
               +exp(IK*R2)*(2.0*ExpRel(1,-IK*R2) -             ExpRel(2,-IK*R2)) / (8.0*M_PI*R2)
               +exp(IK*R3)*(2.0*ExpRel(1,-IK*R3) -             ExpRel(2,-IK*R3)) / (8.0*M_PI*R3);
   };
}
...
{
  RWGEdge *E1 = S1->Edges[ne1];
  RWGEdge *E2 = S2->Edges[ne2];

  double *PV[3], *EV[2];
  EV[0] = S1->Vertices + 3*(E1->iV1);
  EV[1] = S1->Vertices + 3*(E1->iV2);

  PV[0] = S2->Vertices + 3*(E2->iQP);
  PV[1] = S2->Vertices + 3*(E2->iV1);
  PV[2] = S2->Vertices + 3*(E2->iV2);

  cdouble EPI = GetEdgePanelInteraction(PV, EV, k);
 
  if ( E2->iQM != -1 )
   { PV[0] = S2->Vertices + 3*(E2->iQM);
     EPI -= GetEdgePanelInteraction(PV, EV, k);
   };

  return 2.0*E1->Length*E2->Length*EPI / (k*k);
