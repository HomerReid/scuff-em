         ...
         for(q=0; q<4; q++)
          for(r=0; r<4; r++)
           { 
             ...
             if (p>0)
              M->SetEntry(8*ncp + 1, nm, p*X1Powers[p-1]*X2Powers[q]*X3Powers[r]);
             if (q>0)
              M->SetEntry(8*ncp + 2, nm, q*X1Powers[p]*X2Powers[q-1]*X3Powers[r]);
             if (r>0)
              M->SetEntry(8*ncp + 3, nm, r*X1Powers[p]*X2Powers[q]*X3Powers[r-1]);
             if (p>0 && q>0)
              M->SetEntry(8*ncp + 4, nm, p*q*X1Powers[p-1]*X2Powers[q-1]*X3Powers[r]);
             if (p>0 && r>0)
              M->SetEntry(8*ncp + 5, nm, p*r*X1Powers[p-1]*X2Powers[q]*X3Powers[r-1]);
             if (q>0 && r>0)
              M->SetEntry(8*ncp + 6, nm, q*r*X1Powers[p]*X2Powers[q-1]*X3Powers[r-1]);
             if (p>0 && q>0 && r>0)
              M->SetEntry(8*ncp + 7, nm, p*q*r*X1Powers[p-1]*X2Powers[q-1]*X3Powers[r-1]);
             nm++;
           };
      }; 
