...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Point charge on dielectric slab}
\label{PointChargeAboveSlabSection}

To lend insight into the results of Section \ref{},
I here use simple reasoning to study the surface charge
$\sigma$
...
\subsubsection*{Point source above interface}
First assume that the point source lies above the interface,
...
the normal electric fields just above and below the
...
\begin{align*}
\epsilon\subt{A}E_z(\rho,0^+) &= \epsilon\subt{B}E_z(\rho,0^-)
...
\sigma(\rho) &= \pf{\epsilon\subt{A}-\epsilon\subt{B}}
                   {\epsilon\subt{A}+\epsilon\subt{B}}
...
$$ Q\sups{induced}= 2\pi \int_0^\infty \rho \sigma(\rho) d\rho
   = \pf{\epsilon\subt{A} - \epsilon\subt{B}}
       {\epsilon\subt{A} + \epsilon\subt{B}}Q
$$
