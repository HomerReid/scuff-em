...
\begin{figure}
\begin{center}
\resizebox{\textwidth}{!}{\includegraphics{StaticSubstrate.pdf}}
...
\begin{figure}
\begin{center}
\resizebox{\textwidth}{!}{\includegraphics{SigmaLayers.pdf}}
...
&=                G_0(\vb x\subt{D}, \vb x\subt{S}) 
 ...
 + \sum_n G^{\sigma_n}(\vb x\subt{D})
            }_{G\supt{subs}(\vb x\subt{D}, \vb x\subt{S})}
\label{GTotal}
%--------------------------------------------------------------------%
\intertext{where}
%--------------------------------------------------------------------%
...
\intertext{is the bare vacuum Green's function
           [and $G\supt{subs}(\vb x\subt{D},\vb x\subt{S})$ is the correction
...
&\int \frac{qdq}{4\pi } J_0(q\rho)
   \sum_{n}
   \Big[ \epsilon_{m} \xi(q; z_m^+; z_n)
        -\epsilon_{m+1 } \xi(q; z_m^-; z_n)
   \Big]\wt{\sigma}_n(q)
\\
&\qquad
  =\big( \epsilon_{m+1} - \epsilon_{m}\big)
...
\sum_n
 \underbrace{
 \left[
 \frac{ \epsilon_{m} \xi(q; z_m^+; z_n)
        -\epsilon_{m+1  } \xi(q; z_m^-; z_n)
      }{ \epsilon_{m}-\epsilon_m+1}
...
for $\epsilon_2>\epsilon_1$ the surface-charge density induced
by a positive point source is \textit{negative}, as expected.}
%====================================================================%
\numeq{SigmaSingleLayer}
{\wt{\sigma}(q)
   = -\pf{\epsilon_2 - \epsilon_1}{\epsilon_2+\epsilon_1}
...
\numeq{ScriptGSingleLayer}
{
   \left\{\begin{array}{c}
               G^\sigma(\vbrho) \\
   -\partial_x G^\sigma(\vbrho) \\
   -\partial_y G^\sigma(\vbrho) \\
   -\partial_z G^\sigma(\vbrho)
   \end{array}\right\}
   = -\frac{1}{4\pi}
      \pf{\epsilon_2 - \epsilon_1}{\epsilon_2+\epsilon_1}
   \left\{
...
%====================================================================%
Notice that the substrate contribution $G^\sigma$ has the
same $\rho$ dependence as the \text{free} Green's function
$G_0$, with no exponential damping factors; the
total Green's function, obtained by summing substrate and free-space
contributions, looks like the free Green's function but with
an overall multiplicative renormalization:
%====================================================================%
\begin{align}
 G(\vbrho)&= G_0(\vbrho) + G^\sigma(\vbrho)
\nn
 &=\frac{1}{4\pi|\vbrho|}
...
\paragraph{Consequences for BEM matrix assembly}
One consequence of this for {\sc scuff-em} electrostatics
calculations is that, for planar conductors on infinite dielectric
substrates, we don't need to introduce any fancy technology
or evaluate any new integrals to compute BEM matrix elements;
we just use the existing {\sc scuff-em} codes for computing
matrix elements in vacuum and divide the result by
$\epsilon\sups{effective}.$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
which, when added to $G_0$, again amounts to a multiplicative
rescaling of the free-space result.
