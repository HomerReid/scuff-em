...
Consider an infinite homogeneous medium characterized
by spatially uniform $\epsilon$ and $\mu$. If we have
...
&= 
   \int
    \Big\{
     \Gamma\supt{EE}_{ij}(\epsilon, \mu; \omega; \vb x, \vb x^\prime) 
     J_j(\vb x^\prime)
     +
     \Gamma\supt{EM}_{ij}(\epsilon, \mu; \omega; \vb x, \vb x^\prime) 
     M_j(\vb x^\prime)
    \Big\}
    \, d\vb x^\prime
\\
...
&= 
   \int
    \Big\{
     \Gamma\supt{ME}_{ij}(\epsilon, \mu; \omega; \vb x, \vb x^\prime) 
     J_j(\vb x^\prime)
     +
     \Gamma\supt{MM}_{ij}(\epsilon, \mu; \omega; \vb x, \vb x^\prime) 
     M_j(\vb x^\prime)
    \Big\}
    \, d\vb x^\prime.
\end{align*}
...
(\ref{PECEquationBreakdown}) and (\ref{DielectricEquationBreakdown})
...
\subsubsection*{Discretized Version of Equation
                (\ref{DieletricEquationBreakdown}) and its 
                Magnetic Analogue}
...
of object $\mathcal{O}_\gamma$ to find
%====================================================================%
\begin{align}
&\sum_{n=1}^{\texttt{NEdges}(\gamma)}  \bigg\{
   \Big\langle 
         \vb f_{\gamma m} 
   \Big| \BG\supt{EE}(\alpha) + \BG\supt{EE}(\gamma) \Big|
         \vb f_{\gamma n}
   \Big\rangle 
         K_{\gamma n}
   -Z_0
...
\label{DiscretizedDielectricEquation1}
\end{align}
%====================================================================%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
block of matrix elements to the BEM matrix, as follows:
%====================================================================%
\begin{enumerate}
%--------------------------------------------------------------------%
...
\item \textbf{ $\mathcal{O}_\alpha, \mathcal{O}_\beta$ are both
               dielectric but $\mathcal{O}_\alpha \ne \mathcal{O}_\beta$:}
...
interacting through the medium inside the object:

\begin{align*}
 M\Big( \mathcal{I}^K_{\alpha m}, \mathcal{I}^K_{\beta n} \Big) 
  &= \frac{1}{Z_0} \Big\langle 
                   \vb f_{\alpha m} 
                   \Big| \BG\supt{EE}(\vb A) + \BG\supt{EE}(\alpha) \Big|
                   \vb f_{\beta n} 
                   \Big\rangle
\\
 M\Big( \mathcal{I}^N_{\alpha m}, \mathcal{I}^N_{\beta n} \Big) 
  &= \frac{1}{Z_0} \Big\langle 
                   \vb f_{\alpha m} 
                   \Big| \BG\supt{ME}(\vb A) + \BG\supt{ME}(\alpha) \Big|
                   \vb f_{\beta n} 
                   \Big\rangle
\\
 M\Big( \mathcal{I}^K_{\alpha m}, \mathcal{I}^N_{\beta n} \Big) 
  &= \frac{1}{Z_0} \Big\langle 
                   \vb f_{\alpha m} 
                   \Big| \BG\supt{EM}(\vb A) + \BG\supt{EM}(\alpha) \Big|
                   \vb f_{\beta n} 
                   \Big\rangle
\\
 M\Big( \mathcal{I}^N_{\alpha m}, \mathcal{I}^N_{\beta n} \Big) 
  &= \frac{1}{Z_0} \Big\langle 
                   \vb f_{\alpha m} 
                   \Big| \BG\supt{MM}(\vb A) + \BG\supt{MM}(\alpha) \Big|
                   \vb f_{\beta n} 
                   \Big\rangle
\end{align*}
%--------------------------------------------------------------------%
