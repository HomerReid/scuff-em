...
{  \sum_{\mathcal{S}_s \subset \partial \mc R}
   ...
   \end{array}\right)
   \star \left(\begin{array}{cc} \vb K \\ \vb N \end{array}\right)
   dA
   =
  -
...
{  \sum_{\mc S_s \subset \partial \mc R}
   ...
   \Big[ \bmc G(\mc R) \star \bmc C\Big] dA 
    = 
    -\bmc F\incr
}
...
& \sum_{\mc S_s \subset \partial \mc R_1}
   ...
   \int_{\mc S_s}
   \Big[ \bmc G(\mc R_1) \star \bmc C\Big]_{\parallel} dA 
\nn
&\qquad
 -\sum_{\mc S_s \subset \partial \mc R_2} 
   \text{sgn}\big(\mc S_s,\mc R_2\big)
   \int_{\mc S_s}
   \Big[ \bmc G(\mc R_2) \star \bmc C\Big]_{\parallel} dA 
=
...
so two terms on the LHS wind up \textit{adding}, not
subtracting, for that surface.
