...
\subsection*{Nonsingular Edge-Edge Interactions}

\numeq{xxpEdgeEdge}
{
   \vb x = \vb V_0 + u\vb L
   \qquad
   \vb x^\prime = \vb V_0^\prime + u^\prime \vb L^\prime
}
\begin{align}
&\hspace{-1in}
 -\frac{1}{k^2}\int_{\vb L_\alpha} d\vb x \int_{\vb L_\beta} d\vb x^\prime
    \big [\nabla \cdot \vb h(\vb x)        \big] 
    G(\vb x, \vb x^\prime)
    \big [\nabla \cdot \vb h(\vb x^\prime) \big]
\nn
&=
  -\frac{l_\alpha l_\beta}{k^2} \int_0^1 du \, \int_0^1 du^\prime \,
  \frac{e^{ik|\vb x - \vb x^\prime|}}{4\pi |\vb x- \vb x^\prime|}
\label{EdgeEdgeCubature}
\end{align}

\subsection*{Singular Edge-Edge Interaction: Common-Vertex Case }

If $\vb V_0=\vb V_0^\prime$ but $\vb L\ne \vb L^\prime$ 
in (\ref{xxpEdgeEdge}),

\subsection*{Singular Edge-Edge Interaction: Common-Edge Case }

If $\vb V_0=\vb V_0^\prime$ and $\vb L=\vb L^\prime$
in (\ref{xxpEdgeEdge}), the integral in (\ref{EdgeEdgeCubature})
is 

\begin{align*}
&
 -\frac{l}{k^2} \int_0^1 du \, \int_0^1 du^\prime \,
  \frac{e^{ik l(u-u^\prime)}}{4\pi |u-u^\prime|}.
\intertext{Change variables to $s=u+u^\prime$, $t=u-u^\prime$:}
...
% -\frac{l}{k^2} \int_0^1 du \, \int_0^1 du^\prime \,
%  \frac{e^{ik l(u-u^\prime)}}{4\pi |u-u^\prime|}.
%\intertext{Split the unit square into two triangles:}
%&= -\frac{l}{k^2} 
%    \left\{ \int_0^1 du \, \int_0^u du^\prime \,
%            \frac{e^{ik l(u-u^\prime)}}{4\pi |u-u^\prime|}
%   +
%            \int_0^1 du^\prime \, \int_0^{u^\prime} du \,
%            \frac{e^{ik l(u-u^\prime)}}{4\pi |u-u^\prime|}
%    \right\}
%\intertext{In the first (second) integral put 
%             $u^\prime=u\,x, du^\prime=u\,dx$ 
%            ($u=u^\prime x, du=u^\prime dx$):
%          }
%&= -\frac{l}{k^2} 
%    \left\{ \int_0^1 du \, \int_0^1 dx \,
%            \frac{e^{ik lu(1-x)}}{4\pi |1-x|}
%   +
%            \int_0^1 du^\prime \, \int_0^1 dx \,
%            \frac{e^{ik lu^\prime(x-1)}}{4\pi |1-x|}
%    \right\}
%\\
%&= -\frac{2l}{k^2} \text{ Re }
%    \left\{ \int_0^1 du \, \int_0^1 dx \,
