...
double SWPPIThreshold=40.0;

// defined at end for warning-disabling purposes
static double MyX_CT(TMWorkspace *TMW, int i, double x);
static double MyX_CE(TMWorkspace *TMW, int i, double x1, double x2);
static double MyX_CV(TMWorkspace *TMW, int i, double x1, double x2, double x3);

static void MySiAlpha_One(const double *xVec, TMWorkspace *TMW, int WhichCase,
                          int *AlphaMin, int *AlphaMax, cdouble S[7][5]);
static void MySiAlpha_Dot(const double *xVec, TMWorkspace *TMW, int WhichCase,
                          int *AlphaMin, int *AlphaMax, cdouble S[7][5]);
static void MySiAlpha_Cross(const double *xVec, TMWorkspace *TMW, int WhichCase,
                            int *AlphaMin, int *AlphaMax, cdouble S[7][5]);
static void MySiAlpha_DotPlus(const double *xVec, TMWorkspace *TMW, int WhichCase,
                              int *AlphaMin, int *AlphaMax, cdouble S[7][5]);
static void MySiAlpha_ENormal(const double *xVec, TMWorkspace *TMW, int WhichCase,
                              int *AlphaMin, int *AlphaMax, cdouble S[7][5]);

static cdouble MyIn_RP(int n, cdouble GParam, double X);
static cdouble MyIn_EIKROverR(int n, cdouble GParam, double X);
static cdouble MyIn_GradEIKROverR(int n, cdouble GParam, double X);
...
cdouble TaylorDuffy2(TaylorDuffyArgStruct *Args)
{
  ...
  switch(WhichG)
   { case TM_RP:             
       ...
       break;
     case TM_EIKR_OVER_R: 
       TMW->InFunc=MyIn_EIKROverR;
       break;
     case TM_GRADEIKR_OVER_R:
       TMW->InFunc=MyIn_GradEIKROverR;
       break;
     default:
  ...
  switch(WhichH)
   { case TM_ONE:
       TMW->SiAlphaFunc=MySiAlpha_One;
       break;
     case TM_DOT:
       TMW->SiAlphaFunc=MySiAlpha_Dot;
       break;
     case TM_DOTPLUS:
       // note: DOTPLUS is equivalent to 'Dot' + F * 'One' 
       //  where 'F' = -4.0/(GParam*GParam);
       TMW->SiAlphaFunc=MySiAlpha_DotPlus;
       break;
     case TM_CROSS:
       TMW->SiAlphaFunc=MySiAlpha_Cross;
       break;
     case TM_ENORMAL:
       TMW->SiAlphaFunc=MySiAlpha_ENormal;
       break;
   };
...
static cdouble MyExpRel(int n, cdouble Z)
{
  int m;
...
static cdouble MyIn_EMKROverR(int n, double K, double R)
{
  double KR=K*R;

  if ( abs(KR) < 0.1 )
   return exp(-KR)*MyExpRel(n,KR) / ((double)(n)*R);
  else 
   { cdouble Sum, Term;
     int m;
...
static cdouble MyIn_EIKROverR(int n, cdouble K, double R)
{ 
  cdouble KR=K*R;

  if ( imag(KR) > SWPPIThreshold )
   return factorial(n-1) / (R*pow(-II*K*R,n));
  else if ( real(K)==0.0 )
   return MyIn_EMKROverR(n, imag(K), R);
  else if ( abs(KR) < 0.1 )
   return exp(II*K*R)*MyExpRel(n,-II*K*R) / ((double)(n)*R);
  else 
...
static cdouble MyIn_GradEMKROverR(int n, double K, double R)
{
  double KR=K*R;
  double PreFac=((double)(n-1))/((double)(n-2));

  if ( abs(KR) < 0.1 )
   return exp(-KR)*( 1.0 - PreFac*MyExpRel(n-2,KR)) / (R*R*R); 
  else 
   { cdouble Sum, Term;
     int m;
...
static cdouble MyIn_GradEIKROverR(int n, cdouble K, double R)
{ 
  ...
  if ( (imag(KR)) > SWPPIThreshold  )
   ...
   return MyIn_GradEMKROverR(n, imag(K), R);
  else if ( abs(KR) < 0.1 )
   return exp(II*KR)*( 1.0 - PreFac*MyExpRel(n-2,-II*KR)) / (R*R*R); 
  else 
   { cdouble Sum, Term, miKR=-II*KR;
     int m;
     for(Sum=0.0, Term=1.0, m=0; m<n-2; m++)
